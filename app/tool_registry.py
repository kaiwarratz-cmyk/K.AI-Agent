...
TOOL_CATALOG_TREE = {
    ...
    "🧠 MEMORY (mem)": [
        ...
        "mem_save_fact(key, fact, type): Speichert Fakten DAUERHAFT die ein zukünftiger Job sonst neu entdecken müsste. type PFLICHT: device=IP/Port/Gerät, credential=API-Key/Token/Username, path=Tool-/Dateipfad, api_quirk=API-Sonderverhalten, config=Nutzer-Einstellung. NICHT speichern: Standard-Bibliotheken, allgemeines Wissen, temporäre Zustände.",
...
CORE_TOOLS: List[Dict[str, Any]] = [
    ...
    {"type": "function", "function": {"name": "mem_save_fact", "description": "Speichert einen Fakt DAUERHAFT im jobübergreifenden Langzeitgedächtnis. Nur speichern wenn ein zukünftiger Job OHNE diesen Fakt dieselbe Entdeckungsarbeit machen müsste. Typische Beispiele: Tool-Pfade (arduino_cli_path), Hardware-Adressen (esp32_port=COM6, printer_ip=192.168.1.x), API-Sonderverhalten (hue_api_gibt_dict_nicht_list), Nutzer-Einstellungen (preferred_language). NICHT speichern: Standard-Bibliotheken, allgemeines Wissen, temporäre Zustände.", "parameters": {"type": "object", "properties": {"key": {"type": "string", "description": "Eindeutiger Schlüssel, z.B. 'arduino_cli_path', 'esp32_port', 'hue_bridge_ip'."}, "fact": {"type": "string", "description": "Zu speichernder Wert – vollständiger Pfad, IP-Adresse, Konfiguration oder Beschreibung."}, "type": {"type": "string", "enum": ["device", "credential", "path", "api_quirk", "config"], "description": "Kategorie: device=IP/Port/Gerät-ID, credential=API-Key/Token/Username, path=Tool-Pfad/Dateipfad, api_quirk=API-Sonderverhalten aus Trial-and-Error, config=Nutzer-Einstellung/Präferenz"}}, "required": ["key", "fact", "type"]}}},
